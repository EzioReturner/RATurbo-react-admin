...
export function changeTheme(
  ...
  const _className = ['darkTheme', 'lightTheme'].reduce(
    (total: string, _key: string) => {
  ...
  window.less
    ...
    .then(() => {
      ...
      document.body.style.setProperty(
        '--primary-lighten',
        hsl2lighten(hslColor, 8)
      );
      document.body.style.setProperty(
        '--primary-lightener',
        hsl2lighten(hslColor, 20)
      );
      document.body.style.setProperty(
        '--primary-lightener-extra',
        hsl2lighten(hslColor, 33.5)
      );
      document.body.style.setProperty(
        '--primary-darken',
        hsl2darken(hslColor, 8)
      );
      ...
      document.body.style.setProperty(
        '--antd-slider-focus-shadow',
        hsl2fade(hslColor, 0.12)
      );
      document.body.style.setProperty(
        '--antd-select-focus-shadow',
        hsl2fade(hslColor, 0.2)
      );

      document.body.style.setProperty(
        '--antd-slider-active',
        color2tint(rgbColor, 0.5)
      );
      document.body.style.setProperty(
        '--antd-slider-focus',
        color2tint(rgbColor, 0.2)
      );
